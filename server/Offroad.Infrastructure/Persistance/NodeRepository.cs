...
public sealed class NodeRepository : INodeRepository
{
        ...
        CancellationToken ct)
    {
        ...
        var rows = await dbContext.Database
            .SqlQuery<ArenaEntranceRow>($@"
                ...
                scored AS (
                    SELECT
                        ST_X(c.g) AS ""Longitude"",
                        ST_Y(c.g) AS ""Latitude"",
                        COALESCE((
                            SELECT SUM(h.offroad_len_m)
                            FROM gis.heatmap h
                            WHERE h.cell_geom && ST_Expand(c.g, {reachDegrees})
                              AND ST_DWithin(h.cell_geom::geography, c.g::geography, {loopReachMeters})
                        ), 0) AS ""OffroadLengthMeters""
                    FROM candidates c
                )
                SELECT ""Longitude"", ""Latitude"", ""OffroadLengthMeters""
                FROM scored
                WHERE ""OffroadLengthMeters"" >= {minOffroadLengthMeters}
                ORDER BY ""OffroadLengthMeters"" DESC
                LIMIT {limit}")
            .ToListAsync(ct);

        return rows
            .Select(r => new ArenaEntrance(new Coordinate(r.Latitude, r.Longitude), r.OffroadLengthMeters))
            .ToList();
    }
    ...
    private sealed record ArenaEntranceRow(double Longitude, double Latitude, double OffroadLengthMeters);
}
