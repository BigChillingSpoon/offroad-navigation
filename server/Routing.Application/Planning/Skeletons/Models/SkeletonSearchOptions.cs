...
/// The single "avoid private/restricted" switch, mirroring GraphHopper's custom model: if false it
/// excludes both private/forestry edges (HasNoEntry) AND national-park tracks (IsRestricted &&
/// highway == "track"). There is no separate restricted-zones toggle.
/// </param>
/// <param name="MaxGrade">
/// Highest OSM tracktype grade (1-5) the user's vehicle can take; edges with a higher grade are
/// excluded (H1). Defaults to 5 (no-op) until the vehicle profile is wired - a normal car would pass
